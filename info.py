...
auth_channel = environ.get('AUTH_CHANNEL', '-1002593931448')  # Channel/Group ID for force sub (make sure bot is admin)
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002593931448').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002593931448').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
