...
BOT_TOKEN = environ.get('BOT_TOKEN', "7614530743:AAEFyygiVH2PJwtZJjI3KaNEiMFlLt62yJs")
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1989461858').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002212318455').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002290275432'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002334132732'))  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002226636138'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002290275432'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002479054069')  # Channel/Group ID for force sub (make sure bot is admin)
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002334132732').split()]
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002229214716')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002204842444')  # Request channel id (make sure bot is admin)
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002479054069').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002479054069').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-1002290275432'))  # Log channel id (make sure bot is admin)
...
SHORTLINK_API = environ.get('SHORTLINK_API', '088f69f5d57ecf4cdb546b9093f5c9c133fbf2d0')
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/ynvmoviesrequest1')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/ynvmoviesupdates')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/benclass10')
MOVIE_UPDATE_CHANNEL_LNK = environ.get('MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/ynvmoviesupdates')
OWNERID = int(os.environ.get('OWNERID', '1989461858'))  # Replace with the actual admin ID
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/ynvmoviesrequest')  # Support group link (make sure bot is admin)
